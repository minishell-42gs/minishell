...
/*   Updated: 2026/07/28 17:56:20 by taegokim         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_token_node	*create_impl(t_token_factory *this, const char *_value)
{
	t_token_node	*token;
	char			*value;
	t_token_type	type;

	token = ft_calloc(1, sizeof(t_token_node));
	if (!token)
		return (NULL);
	...
	if (token_node_init(token, type, value) != OK)
