...
현재 factory는 정확히 `"|"`만 `TOKEN_PIPE`로 분류한다. 다음 기능을 정하기 전에
요구사항을 한 문장으로 적고, 그 문장을 테스트 이름으로 옮겨 보자. 예를 들어
빈 문자열이나 `NULL`을 factory가 어떻게 처리해야 하는지 먼저 결정한 다음 실패
테스트를 작성할 수 있다.

아직 정하지 않은 행동을 현재 구현에 맞춰 테스트로 고정하지 않는 것도 중요하다.
테스트는 구현의 복사본이 아니라 우리가 원하는 계약이어야 한다.

실행 방법과 현재 테스트 목록은 [README.md](README.md)를 참고한다.
