...
/* 각 테스트 전에 facade, env, executor, 명령 리스트를 초기화하고
 * 종료 코드 저장소를 0으로 되돌린다. */
void	setUp(void)
{
...
void	test_executor_init_sets_reference(void)
{
	TEST_ASSERT_EQUAL_PTR(&g_env_list, g_executor.env_list);
	TEST_ASSERT_NOT_NULL(g_executor.destroy);
}

/* 비어 있는 명령 리스트는 아무것도 실행하지 않고 OK를 반환하며,
 * 호출자가 가진 종료 코드를 건드리지 않아야 한다 (bash의 빈 입력과 동일). */
...
/* 정상 종료한 명령의 종료 코드 0이 out 파라미터로 전달되는지 확인한다. */
...
/* 연속 실행 시 종료 코드가 마지막 명령의 코드로 덮어써지는지 확인한다. */
...
{
	UNITY_BEGIN();
	RUN_TEST(test_executor_init_sets_reference);
